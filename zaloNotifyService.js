...
async function notifyDocumentCompleted(doc, approverUser, viewUrl = '') {
  ...
  console.log(`[ZaloNotify] Đang gửi thông báo HOÀN TẤT KÝ DUYỆT (hasSchoolSeal=${isRealSchoolSeal}) tới SĐT tác giả: ${authorPhone || 'Chưa có SĐT'}`);
...
async function notifyDocumentBghApproved(doc, approverUser, viewUrl = '') {
  ...
  console.log(`[ZaloNotify] Đang gửi thông báo BGH PHÊ DUYỆT (CHỜ ĐÓNG DẤU) tới SĐT tác giả: ${authorPhone || 'Chưa có SĐT'}`);
  ...
  return await sendWebhookPost({
    ...
    authorPhone: authorPhone,
    approverName: approverName,
    viewUrl: viewUrl || (doc.driveInfo ? doc.driveInfo.viewUrl : ''),
    hasSchoolSeal: false,
    isSchoolSeal: false,
    requiresSeal: true,
...
async function notifyDocumentForwarded(doc, senderUser, targetUserId) {
  ...
  return await sendWebhookPost({
    ...
    reportCategory: doc.reportCategory || (doc.requiresSeal ? 'SCHOOL' : 'INTERNAL')
  });
}
